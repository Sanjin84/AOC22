...
def move(num_items, from_stack, to_stack):
    global stacks
    #remove num_items from from_stack and add them to to_stack
    stacks[to_stack].extend(stacks[from_stack][-num_items:])
    #remove num_items from from_stack
    stacks[from_stack] = stacks[from_stack][:-num_items]
